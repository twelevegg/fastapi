...
### 1. 상담 가이드 및 마케팅 (Guidance & Marketing)
- **통화 STT**: 음성 통화 내용을 실시간으로 채팅화 하여 보여줍니다.
- **실시간 가이드**: 상담 진행 중 AI가 대화 문맥을 파악하여 추천 답변이나 필요한 정보를 실시간으로 제공합니다.
- **마케팅**: 상담 진행 중 AI가 대화 문맥을 파악하여, 업셀링 혹은 해지방어 전략으로 상품을 추천합니다.
  
### 2. AI 상담 시뮬레이션 (Simulation)
- **실시간 대화 시뮬레이션**: WebSocket을 활용하여 상담사와 고객(AI) 간의 실시간 음성/텍스트 대화를 시뮬레이션합니다.
- **다양한 시나리오**: 상담 시나리오(예: 요금제 변경, 결합 상품 문의 등)를 바탕으로 정교한 고객 페르소나를 AI가 연기합니다.
- **자동 평가**: 시뮬레이션 종료 후 상담 내용을 분석하여 응대 적절성을 평가합니다.

### 3. 교육 콘텐츠 자동 생성 (Edu-Job)
