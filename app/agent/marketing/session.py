...
class CustomerProfile:
    ...
    def from_dict(data: Dict[str, Any]) -> "CustomerProfile":
        ...
        prof = CustomerProfile(
            ...
            mobile_plan=s("rate_plan")
            or s("mobile_plan")
            or s("요금제명"),  # Map 'rate_plan' from agent.py
    ...
    def _derive_signals(p: CustomerProfile) -> List[str]:
        ...
        if p.monthly_fee_won:
            if p.monthly_fee_won >= 80000:
                s.append(
                    "고액 요금 납부자(VIP: 멤버십/프리미엄 혜택 강조하여 이탈 방어)"
                )
...
class QdrantSearchEngine:
        ...
        return models.Filter(
            must=[
                models.FieldCondition(
                    key=self.category_key, match=models.MatchValue(value=category)
                )
            ]
        )
    ...
    def semantic(
        ...
        return self._to_items(
            self.vs_dense.similarity_search_with_score(
                query=query, k=k, filter=self._filter(category)
            )
        )
    ...
    def keyword(
        ...
        return self._to_items(
            self.vs_sparse.similarity_search_with_score(
                query=query, k=k, filter=self._filter(category)
            )
        )
    ...
    def hybrid(
        ...
        return self._to_items(
            self.vs_hybrid.similarity_search_with_score(
                query=query, k=k, filter=self._filter(category)
            )
        )
    ...
    def staged_category_search(
        ...
        if not cats:
            return self.fused_search(
                query, final_k=final_k, k_each=max(per_category_k, 6), category=None
            )
        ...
        for c in cats:
            per.append(
                self.fused_search(
                    query, final_k=per_category_k, k_each=per_category_k, category=c
                )
            )
...
def build_context(
    items: List[RetrievedItem], max_chars: int = 8500, per_doc_chars: int = 850
...
def quick_router(dialogue: str, customer: CustomerProfile) -> Dict[str, Any]:
    ...
    if any(
        k in t
        for k in ["성함", "본인", "명의", "인증", "주소지", "연락주신 번호", "확인"]
    ):
        stage = "verification"
        reasons.append("본인확인/정보확인 발화 감지")
    if any(
        k in t
        for k in ["동의", "녹취", "개인정보", "위탁", "보관", "약관", "필수적으로 필요"]
    ):
        stage = "consent"
    ...
    db_signal = (
        any(
            any(k in s for k in ["약정 만료", "초과요금", "할인 미적용", "결합"])
            for s in customer.signals
        )
        if customer.signals
        else False
    )
    ...
    complaint = any(
        w in t for w in ["불만", "끊김", "느려", "장애", "환불", "오류", "안돼", "문제"]
    )
    ...
    if stage in ["verification", "consent"] and not churn:
        if upsell:
            reasons.append(
                "verification/consent 단계지만 고객이 결합/요금제 흐름 → 제한적 안내 가능"
            )
        else:
            marketing_needed = False
            mtype = "support_only"
    ...
    return {
        "call_stage_hint": stage,
        "marketing_needed_hint": marketing_needed,
        "marketing_type_hint": mtype,
        "reasons": reasons[:6],
    }
...
class OpenAICompatibleLLM:
    def __init__(self, timeout: int = 60):
        ...
        self.allow_fallback = (
            os.environ.get("LLM_ALLOW_FALLBACK") or ""
        ).strip().lower() in ["1", "true", "yes", "y"]

        if not self.base_url or not self.api_key or not self.model:
            raise RuntimeError(
                "LLM env missing: LLM_BASE_URL/LLM_API_KEY/LLM_MODEL (or OPENAI_*)"
            )
    ...
    async def chat_json(
        ...
        async def _post(cx: httpx.AsyncClient, pl: Dict[str, Any]) -> httpx.Response:
            return await cx.post(
                f"{self.base_url}/chat/completions", headers=headers, json=pl
            )
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as cx:
            ...
            if finish_reason == "length":
                ...
                content = (
                    ((choice2.get("message") or {}).get("content")) or ""
                ).strip()
            ...
            try:
                ...
                content3 = (
                    ((choice3.get("message") or {}).get("content")) or ""
                ).strip()
    ...
    async def chat_json(
        ...
        return {
            ...
            "decision": {
                ...
                "next_questions": [
                    "요청하시는 목적(결합/요금/해지/혜택)을 확인 부탁드립니다."
                ],
                "next_actions": [
                    {
                        "priority": 1,
...
class MarketingSession:
    ...
    def add_turn(
        self, speaker: str, transcript: str, turn_id: Optional[int] = None
    ) -> None:
        sp = "customer" if speaker == "customer" else "agent"
        if turn_id is None:
            turn_id = (self.turns[-1].turn_id + 1) if self.turns else 1
        self.turns.append(
            Turn(turn_id=turn_id, speaker=sp, transcript=transcript or "")
        )
    ...
    async def prefetch(self, trigger_chunk: str) -> None:
        ...
        q_items = self.qdrant.staged_category_search(
            query=trigger_chunk, final_k=5, categories=cats
        )
    ...
    def dialogue_text(self, last_n: int = 14) -> str:
        part = (
            self.turns[-last_n:] if last_n and len(self.turns) > last_n else self.turns
        )
        lines = []
        for t in part:
    ...
    def build_query(self) -> str:
        ...
        parts += [
            p
            for p in [
                self.customer.mobile_plan,
                self.customer.internet_plan,
                segment,
                sig_text,
                dialog,
            ]
    ...
    async def step(self, session_id: str = "legacy_session") -> Dict[str, Any]:
        ...
        config = {"configurable": {"thread_id": session_id, "session": self}}
        ...
        try:
            ...
            return {
                ...
                "decision": {
                    ...
                    if final_state.get("next_actions")
                    else [
                        {
                            "agent_script": {
                                "opening": final_state.get("agent_script", "")
                            }
                        }
                    ]
                },
                "product_recommendations": final_state.get(
                    "product_candidates", []
                ),  # Schema match?
        ...
        try:
            ...
            if isinstance(dec, dict):
                ...
                if acts and isinstance(acts, list):
                    return (
                        acts[0].get("agent_script", {}).get("proposal")
                        or acts[0].get("agent_script", {}).get("opening")
                        or ""
                    )
        except:
            pass
...
_shared_qdrant_engine: Optional["QdrantSearchEngine"] = None
...
def get_shared_qdrant_engine() -> "QdrantSearchEngine":
    global _shared_qdrant_engine
    if _shared_qdrant_engine is not None:
        return _shared_qdrant_engine
    ...
    _shared_qdrant_engine = QdrantSearchEngine(
        client=client,
        collection="cs_guideline",
        vector_name="dense",
...
def build_session(
    ...
    # 2. Vector DB (Qdrant) - shared singleton to avoid reloading embeddings
    qengine = get_shared_qdrant_engine()
